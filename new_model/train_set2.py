...
parser.add_argument('--increment_epochs', type = int, default = 10, help = 'number of epochs to increase after each set')
...
parser.add_argument('--total_classes', type = int, default = 50, help = 'total number of classes to train over')
...
def train_model(dataset=dataset, save_dir=save_dir, load_dir = load_dir, num_classes=num_classes, lr=lr,
    ...
    classifier = Modified_Classifier(num_classes = 40)
    ...
    init_classes = 40
    iters = total_classes//num_classes
    ...
    for i in range(1):
        ...
        if (i == 0):
            ...
            print('new out features: ', init_classes + num_classes)
            classifier.classifier_out = nn.Linear(in_features, init_classes + num_classes, bias = False)
            kaiming_normal_init(classifier.classifier_out.weight)
            classifier.classifier_out.weight.data[:init_classes] = weights
            #model1.eval()
            #classifier1.eval()
        ...
        train_dataset = video_dataset(train = True, classes = all_classes[init_classes:init_classes + num_classes])
        train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=4)
        test_dataset = video_dataset(train = False, classes = all_classes[init_classes:init_classes + num_classes])
        test_dataloader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, num_workers=4)

        old_train_dataset = old_video_dataset(train = True, classes = all_classes[:init_classes], num_classes = init_classes, samples = 10)
        old_train_dataloader = DataLoader(old_train_dataset, batch_size=batch_size, shuffle=True, num_workers=4)

        old_test_dataset = video_dataset(train = False, classes = all_classes[:init_classes])
        ...
        optimizer1 = torch.optim.Adam(train_params, lr=lr)
            ...
            for epoch in range(num_epochs):
                ...
                for indices, inputs, labels in (trainval_loaders["train"]):
                    ...
                    if (i == 0):
                        ...
                        logits_dist = logits[:, :init_classes]
                ...
                writer.add_scalar('data/train_acc_epoch {}'.format(i), epoch_acc, epoch)
                ...
                writer.add_scalar('data/old_loss_epoch {}'.format(i), epoch_loss, epoch)
                writer.add_scalar('data/old_train_acc_epoch {}'.format(i), epoch_acc, epoch)

                print("[test] Epoch: {}/{} Old Dataset Training Acc: {}".format(epoch+1, nEpochs, epoch_acc))
                ...
                if useTest and epoch % test_interval == (test_interval - 1):
                    ...
                    writer.add_scalar('data/test_acc_epoch {}'.format(i), real_epoch_acc, epoch)
                    ...
                    if (i == 0):
                        ...
                        writer.add_scalar('data/old_acc_epoch {}'.format(i), real_epoch_acc, epoch)

                        print("[test] Epoch: {}/{} Old Dataset Acc: {}".format(epoch+1, num_epochs, real_epoch_acc))
        ...
        init_classes = init_classes + num_classes
